...
;; ------------------------------
;; It is likely that a file-local major mode is actually safe, even if long
;; lines are present, and consequently these files are treated as special
;; cases.  When a file-local 'mode' is present, the function defined by the
;; `so-long-file-local-mode-function' user option is called.  The default
;; value will cause the `overrides-only' action to be used instead of the
;; `so-long-mode' action, if the latter was going to be used for this file.
...
If t, then so-long affects all modes."
  ;; symbol, not function, as may be unknown => mismatch.
