...
(defvar so-long-map (make-sparse-keymap)
  "Keymap for `so-long'.")

;; (defvar so-long-map (let ((map (make-sparse-keymap)))
...
(defun so-long-menu-add ()
  "Add a menu of so-long options.  Called when `so-long-line-detected-p'."
  ;; (define-key so-long-map [menu-bar so-long]
  ;;   (cons "So Long" (make-sparse-keymap "So Long")))
  ;;
  ;; (define-key so-long-map [menu-bar so-long hide-all]
...
;; (define-key menu-bar-edit-menu [so-long]
;;   (list 'menu-item "So Long" so-long-mode-line-menu))

(defvar so-long-mode-line-menu
  (let ((map (make-sparse-keymap "So Long")))
    ...
    ;; (define-key map (kbd "<mode-line> <mouse-1>") '("Hello" . forward-word))
    ;; (define-key map [menu-bar so-long hello] '("Hello" . forward-word))
    (define-key map [hello]
      '(menu-item "Hello" forward-word
                  :enable t
                  :visible t
                  :help "Help"
                  ))
...
(defun so-long-mode-line-info ()
  ...
  '(so-long-mode-line-label
    ("" (:eval (propertize so-long-mode-line-label
                           ...
                           ;; 'help-echo "mouse-1: do the thing"
  ...
  (let ((remembered (so-long-original variable :exists)))
    (when remembered
      ...
      (unless (and (equal (symbol-value variable) (cadr remembered))
                   (not (nth 3 remembered))) ;; originally global
...
`so-long-file-local-mode-function'."
  ...
  (and (eq (ad-get-arg 0) t)
       (when ad-return-value ; A file-local mode was set.
...
(defun so-long-enable ()
  ...
  (define-key-after (current-global-map) [menu-bar so-long]
    `(menu-item "So Long" ,so-long-mode-line-menu
