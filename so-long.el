...
;; chosen the major mode for a buffer), `hack-local-variables' (in order that
;; we may inhibit our functionality when a file-local mode is set).  In Emacs
;; versions < 26, we also advise `hack-one-local-variable' (in order that we
;; may prevent a file-local mode from restoring the original major mode if we
;; had changed it).
;;
...
(put 'so-long-file-local-mode-function 'permanent-local t)
...
want to increase `so-long-max-lines' to allow for possible comments."
  (setq so-long-line-detected-p nil)
  (let ((count 0) start)
    (save-excursion
      (goto-char (point-min))
...
type \\[so-long-mode-revert], or else re-invoke it manually."
  ...
  ;; `so-long', so duplicate the critical settings.  We could use this same test
  ...
  (unless so-long--calling
    ...
    (setq so-long-function 'so-long-mode)
    (setq so-long-revert-function 'so-long-mode-revert))
...
(defun so-long-enable ()
  ...
  (define-key-after (current-global-map) [menu-bar so-long]
    `(menu-item "So Long" nil
                ...
                :visible (or so-long-line-detected-p
                             (derived-mode-p 'so-long-mode))
                :filter ,(lambda (_cmd) (so-long-menu))))
