...
(defun so-long-mode-line-menu ()
  ...
  (let ((map (make-sparse-keymap "So Long")))
    ...
    ;; `so-long-replace-action' over `so-long-action-alist'.
...
;; FIXME: Find out how to identify the mouse event which triggered a
;; given menu item, so that we can *then* establish which window/buffer
