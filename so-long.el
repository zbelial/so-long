...
(defun so-long-restore-variable (variable)
  ...
  (let* ((remembered (so-long-original variable :exists))
    ...
    (if (or originally-global (not remembered))
        ...
        ;;
        ;; n.b. It's possible that the global value has *changed* in the
        ;; interim; but we can't know whether it's best to use the new global
        ;; value, or keep the old value as a buffer-local value, so we keep it
        ;; simple.
