...
If t, then so-long affects all modes.  Note that this is only useful with a
custom `so-long-predicate', as many modes (archives and binary files, for
example) are safe when they contain long lines, and should not be targeted."
