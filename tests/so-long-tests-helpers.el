...
(defun so-long-tests-assert-and-revert (action &optional ignore-so-long-action)
  "Assert ACTION, revert it, and then assert the revert."
  (so-long-tests-assert-active action ignore-so-long-action)
  (so-long-revert)
  (so-long-tests-assert-reverted action ignore-so-long-action))
